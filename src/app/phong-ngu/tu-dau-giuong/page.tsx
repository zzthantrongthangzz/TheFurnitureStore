...
function NightstandPageContent() {
  // LỌC DỮ LIỆU CẤP 3: Tủ đầu giường
  const nightstandProducts = mockProducts.filter(
    (p) => p.category === "phong-ngu" && p.subCategory === "tu-dau-giuong",
  );
  ...
  const {
    ...
    paginatedProducts,
    totalCount,
  } = useProductFilter(nightstandProducts);
  ...
  const toggleSection = (key: "price" | "color" | "size") =>
    ...
    <main className="min-h-screen bg-white text-gray-800">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="flex text-sm text-gray-500 mb-4 space-x-2">
          ...
          <span>/</span>
          <Link href="/phong-ngu" className="hover:text-orange-500 transition">
            Phòng ngủ
          </Link>
          <span>/</span>
          <span className="text-gray-900 font-medium">Tủ Đầu Giường</span>
        </nav>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-[240px] shrink-0">
            <div className="sticky top-24 space-y-5">
              ...
              >
                <div className="space-y-3 text-sm text-gray-600">
                  {["Gỗ tự nhiên", "Nâu", "Xám", "Be", "Trắng", "Đen"].map(
                    (color) => (
                      <label
                        key={color}
                        className="flex items-center space-x-3 cursor-pointer group"
              ...
              {/* Cập nhật các size thông dụng của tủ đầu giường */}
              <FilterAccordion
                title="Kích thước"
