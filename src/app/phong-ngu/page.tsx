...
// ĐỔI LIST DANH MỤC THÀNH CÁC TÙY CHỌN CẤP 3 CỦA PHÒNG NGỦ
const BEDROOM_CATEGORIES = [
  { label: "Combo Phòng Ngủ", val: "combo-phong-ngu" },
  { label: "Giường Ngủ", val: "giuong-ngu" },
  { label: "Tủ Quần Áo", val: "tu-quan-ao" },
  { label: "Bàn Trang Điểm", val: "ban-trang-diem" },
  { label: "Tủ Đầu Giường", val: "tu-dau-giuong" },
  { label: "Nệm", val: "nem" },
];
...
export default function BedroomPage() {
  // LỌC DỮ LIỆU: Chỉ lấy các sản phẩm thuộc danh mục "phong-ngu"
  const bedroomProducts = mockProducts.filter(
    (p) => p.category === "phong-ngu",
  );
  ...
  const {
    ...
    paginatedProducts,
    totalCount,
  } = useProductFilter(bedroomProducts);
              ...
              {filters.categories.length > 0 && (
                <div className="flex items-center flex-wrap gap-2">
                  ...
                  {filters.categories.map((val) => (
                    ...
                    >
                      {/* Đổi thành BEDROOM_CATEGORIES */}
                      <span>
                        {BEDROOM_CATEGORIES.find((c) => c.val === val)?.label ||
                          val}
                      </span>
                      <button
