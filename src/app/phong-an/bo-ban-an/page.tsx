...
function DiningSetPageContent() {
  // LỌC DỮ LIỆU CẤP 3: Bộ Bàn Ăn
  const diningSetProducts = mockProducts.filter(
    (p) => p.category === "phong-an" && p.subCategory === "bo-ban-an",
  );
  ...
  const {
    ...
    paginatedProducts,
    totalCount,
  } = useProductFilter(diningSetProducts);
    ...
    <main className="min-h-screen bg-white text-gray-800">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="flex text-sm text-gray-500 mb-4 space-x-2">
          ...
          <span>/</span>
          <Link href="/phong-an" className="hover:text-orange-500 transition">
            Phòng ăn
          </Link>
          <span>/</span>
          <span className="text-gray-900 font-medium">Bộ Bàn Ăn</span>
        </nav>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-[240px] shrink-0">
            <div className="sticky top-24 space-y-5">
              ...
              >
                <div className="space-y-3 text-sm text-gray-600">
                  {["Gỗ tự nhiên", "Nâu", "Xám", "Trắng", "Đen"].map(
                    (color) => (
                      <label
                        key={color}
                        className="flex items-center space-x-3 cursor-pointer group"
              ...
              <FilterAccordion
                title="Kích thước (Chiều dài bàn)"
                isOpen={openSections.size}
                onToggle={() => toggleSection("size")}
              >
                <div className="space-y-3 text-sm text-gray-600">
                  {["1m2", "1m4", "1m6", "1m8", "2m"].map((size) => (
                    <label
                      key={size}
                      className="flex items-center space-x-3 cursor-pointer group"
                    >
