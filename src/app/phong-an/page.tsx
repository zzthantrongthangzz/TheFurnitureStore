...
const DINING_ROOM_CATEGORIES = [
  { label: "Bộ Bàn Ăn", val: "bo-ban-an" },
  { label: "Bàn Ăn", val: "ban-an" },
  { label: "Ghế Ăn", val: "ghe-an" },
  { label: "Tủ Bếp", val: "tu-bep" },
  { label: "Tủ Phòng Ăn", val: "tu-phong-an" },
];
...
function DiningRoomPageContent() {
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (!abortController.signal.aborted) {
          const validSubCats = DINING_ROOM_CATEGORIES.map((c) => c.val);

          const diningRoomProducts = productsArray.filter((p: Product) => {
          ...
          const formattedData = diningRoomProducts.map((p: Product) => {
            const sub = (p.subCategory || "").toLowerCase();
            return {
              ...p,
              ...
              {filters.categories.length > 0 && (
                <div className="flex items-center flex-wrap gap-2">
                  ...
                  {filters.categories.map((val) => (
                    ...
                    >
                      <span>
                        {DINING_ROOM_CATEGORIES.find((c) => c.val === val)
                          ?.label || val}
                      </span>
                      <button
          ...
          <section className="flex-1">
            {paginatedProducts.length === 0 ? (
              <div className="py-20 text-center text-gray-500 border-2 border-dashed border-gray-100 rounded-3xl">
                Không tìm thấy sản phẩm Phòng Ăn nào phù hợp.
              </div>
            ) : (
