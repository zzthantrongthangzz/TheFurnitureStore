...
import { 
  DollarSign, ShoppingBag, Package, TrendingUp, Clock, Filter
} from "lucide-react";
import { 
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white p-4 border border-gray-100 shadow-lg rounded-xl max-w-xs">
          <p className="font-bold text-gray-800 mb-2 line-clamp-2">{payload[0].payload.name}</p>
          <p className="text-blue-600 text-sm">Đã bán: <span className="font-bold">{payload[0].value}</span> sản phẩm</p>
          <p className="text-green-600 text-sm">Doanh thu: <span className="font-bold">{payload[0].payload.revenue.toLocaleString("vi-VN")}đ</span></p>
        </div>
      );
    }
    return null;
    ...
    <div className="p-2 md:p-6 space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Phân tích hiệu suất kinh doanh của cửa hàng</p>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 xl:gap-6">
            ...
            <div className="bg-white p-5 xl:p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-3 xl:gap-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs xl:text-sm font-medium text-gray-500 mb-1 truncate">Doanh thu</p>
                <h3 
            ...
            <div className="bg-white p-5 xl:p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-3 xl:gap-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs xl:text-sm font-medium text-gray-500 mb-1 truncate">Tổng đơn hàng</p>
                <h3 
                  className="text-xl xl:text-2xl font-black text-gray-900 truncate"
            ...
            <div className="bg-white p-5 xl:p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-3 xl:gap-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs xl:text-sm font-medium text-gray-500 mb-1 truncate">Chờ xác nhận</p>
                <h3 
                  className="text-xl xl:text-2xl font-black text-gray-900 truncate"
            ...
            <div className="bg-white p-5 xl:p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-3 xl:gap-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs xl:text-sm font-medium text-gray-500 mb-1 truncate">TB / Đơn hàng</p>
                <h3 
                  className="text-xl xl:text-2xl font-black text-gray-900 truncate"
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                {data.revenueChartData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={data.revenueChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                      ...
                      <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#6b7280' }} tickFormatter={(val) => `${(val / 1000000).toFixed(0)}M`} dx={-10} />
                      <Tooltip content={<CustomRevenueTooltip />} />
                      <Area type="monotone" dataKey="total" stroke="#f97316" strokeWidth={3} fillOpacity={1} fill="url(#colorRevenue)" />
                    </AreaChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="h-full flex items-center justify-center text-gray-400 text-sm">Chưa có dữ liệu giao dịch trong khoảng thời gian này.</div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col">
              <h2 className="text-lg font-bold text-gray-900 mb-6">Tỷ trọng trạng thái</h2>
              <div className="flex-1 h-[250px]">
                {data.orderStatusData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={data.orderStatusData} cx="50%" cy="50%" innerRadius={70} outerRadius={100} paddingAngle={5} dataKey="value" stroke="none">
                        {data.orderStatusData.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={entry.color} />
                        ))}
                      </Pie>
                      <Tooltip formatter={(value: any) => [`${value} đơn`, "Số lượng"]} contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
                      <Legend verticalAlign="bottom" height={36} iconType="circle" />
                    </PieChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="h-full flex items-center justify-center text-gray-400 text-sm">Chưa có đơn hàng.</div>
                )}
              </div>
            </div>
            ...
            <div className="xl:col-span-3 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              <h2 className="text-lg font-bold text-gray-900 mb-6 flex items-center gap-2">
                <Package className="w-5 h-5 text-blue-500" /> Top 5 Sản phẩm bán chạy nhất
              </h2>
              <div className="h-[300px] w-full">
                {data.topProductsData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data.topProductsData} layout="vertical" margin={{ top: 0, right: 30, left: 0, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#f3f4f6" />
                      <XAxis type="number" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#6b7280' }} />
                      <YAxis type="category" dataKey="name" width={200} axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#4b5563', fontWeight: 500 }} />
                      <Tooltip cursor={{ fill: '#f3f4f6' }} content={<CustomProductTooltip />} />
                      <Bar dataKey="sold" fill="#3b82f6" radius={[0, 4, 4, 0]} barSize={24} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="h-full flex items-center justify-center text-gray-400 text-sm">Chưa có dữ liệu sản phẩm bán ra.</div>
                )}
              </div>
            </div>
