        ...
        if (res.ok) {
          ...
          const getAttr = (keyName: string) => {
            const attr = data.attributes?.find(
              (a: ProductAttribute) =>
                a.key.toLowerCase() === keyName.toLowerCase(),
            );
      ...
      >
        ...
        <div className="flex justify-end">
          <button
            type="submit"
            disabled={isLoading}
            className={`flex items-center gap-2 px-8 py-3 rounded-lg text-white font-bold transition-colors ${isLoading ? "bg-gray-400" : "bg-orange-600 hover:bg-orange-700"}`}
          >
            {isLoading ? (
              "Đang lưu..."
            ) : (
              <>
                <Save size={20} /> Cập Nhật Sản Phẩm
              </>
            )}
          </button>
        </div>
